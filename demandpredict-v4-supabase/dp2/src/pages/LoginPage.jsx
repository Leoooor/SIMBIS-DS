...
export default function LoginPage() {
  ...
  const submit = async () => {
    ...
    if (mode === "register") {
      ...
      if (!form.businessName)             { setError("Nama UMKM wajib diisi.");       setBusy(false); return; }
      ...
      const r = await register(form.name, form.businessName, form.email, form.password);
      if (!r.ok) setError(r.error);
      else setInfo("Akun dibuat! Cek email untuk konfirmasi, lalu login.");
    } else {
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono', monospace" }}
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="bg-[#0d1221] border border-slate-800/60 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {mode === "register" && <>
              ...
              <div>
                <label className="text-[10px] text-slate-500 uppercase tracking-widest block mb-1">Nama UMKM</label>
                <input value={form.businessName} onChange={set("businessName")} placeholder="Toko Berkah Jaya"
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-sky-500/50"/>
              </div>
            </>}
            ...
            {error && <div className="bg-red-900/20 border border-red-800/40 rounded-lg px-3 py-2 text-xs text-red-300">{error}</div>}
            {info  && <div className="bg-emerald-900/20 border border-emerald-800/40 rounded-lg px-3 py-2 text-xs text-emerald-300">{info}</div>}
        ...
        <p className="text-center text-[10px] text-slate-600 mt-4">Data tersimpan online via Supabase.</p>
      </div>
    </div>
  );
}
