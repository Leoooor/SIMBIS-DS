    ...
    <aside className="flex flex-col w-56 bg-[#0d1221] border-r border-slate-800/60 h-full">
      ...
      {user && (
        <div className="px-4 py-3 border-b border-slate-800/60">
          ...
          <div className="text-xs text-slate-300 font-medium mt-0.5 truncate">{profile?.name || user.email}</div>
          {profile?.business_name && (
            <div className="text-[10px] text-sky-400/70 truncate">{profile.business_name}</div>
          )}
        </div>
      )}
      ...
      <div className="p-4 border-t border-slate-800/60 space-y-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-emerald-400">Online · Supabase</span>
        </div>
